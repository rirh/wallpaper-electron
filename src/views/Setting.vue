  ...
  <div>
    ...
    <div class="app">
      <div class="wapper">
        ...
        <div>
          <select id="s">
            <option>每小时（网络随机下载）</option>
            <option>每日（网络随机下载）</option>
            <option>每小时（本地已下载）</option>
            <option>每日（本地已下载）</option>
          </select>
        </div>
        <br />
        <br />
        ...
        <div class="row" :title="path">
          ...
          <button class="row-btn">更改</button>
        </div>
      </div>
      <!-- <hr /> -->
      <div class="end">
        <span>关于我们</span>
        <span @click="handle_face_back">反馈建议</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    console.log(store);
    new Worker("../worker/checkWapper.js", { type: "module" });
    const val = window.ipcRenderer.sendSync("update-auto-lunch-app", "");
    this.start = val;
    this.path = `${window.os.homedir()}${
